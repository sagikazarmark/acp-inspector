...
| Prompt content beyond text — an `image`, `audio` or `embeddedContext` composer | Deferred with a ring of its own owed. The three `promptCapabilities` rows are advertisements this tool cannot drive at all, which is why they carry no fourth fact ([§7.7](#77-the-fourth-ring-what-was-driven)). Note the MUST here binds *this client*: the schema says a client "MUST adapt its interface according to `PromptCapabilities`", which sending only text satisfies by offering less | [#96](https://github.com/sagikazarmark/dioxus-chat.orig/issues/96) |
