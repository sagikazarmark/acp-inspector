...
| Audio and embedded-context prompt input | Deferred. Image input is implemented by [#11](https://github.com/sagikazarmark/acp-inspector/issues/11): one signature-identified PNG/JPEG/GIF/WebP up to 5 MiB, original bytes, preview/removal, image-only or text-plus-image. Its Affordance is gated on `promptCapabilities.image`; its row records the driven outcome. Audio and embeddedContext still have no Affordance. | [#96](https://github.com/sagikazarmark/dioxus-chat.orig/issues/96), [#11](https://github.com/sagikazarmark/acp-inspector/issues/11) |
