...
| HTTP/SSE MCP definitions | Deferred; `mcpCapabilities.http` and `.sse` still have no Affordance. Stdio definitions are baseline ACP and now have a structured editor before Launch and in Sessions: name, absolute executable path, ordered arguments and environment pairs. One in-memory draft supplies all new/load/resume routes and survives explicit reconnect; edits affect subsequent opens only and never enter recent commands or persistent storage. Validation precedes Connection/Session changes, preserves empty arguments/values and duplicates, and does not inspect the Agent's filesystem. Each opening snapshots the draft; the Agent owns execution, so a setup answer is not individual MCP connectivity evidence. New/load serialize an empty list; resume omits it under the pinned schema. | [#96](https://github.com/sagikazarmark/dioxus-chat.orig/issues/96), [#18](https://github.com/sagikazarmark/acp-inspector/issues/18) |
